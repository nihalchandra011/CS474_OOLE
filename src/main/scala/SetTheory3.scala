...
object SetTheory3:
  ...
  private val Binding: mutable.Map[String, Any] = mutable.Map("var"->10, "str"->"string", "aSetName"->mutable.Set(1,2,3,4,5), "setX"->mutable.Set(2,4,6), "set1"->mutable.Set(10,20,30), "set2"->mutable.Set(30,40,50))
      ...
      this match {
        ...
        case Field(fieldName) =>
          if (ab("ab")) {
            ...
            fieldMapping(AbstractClassStack.top) += fieldName -> 0
          }
          else {
            fieldMapping += ClassStack.top -> Map()
            fieldMapping(ClassStack.top) += fieldName -> 0
          }
          fieldMapping
